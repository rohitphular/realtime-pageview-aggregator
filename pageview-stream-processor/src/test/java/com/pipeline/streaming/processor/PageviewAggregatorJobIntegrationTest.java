...
class PageviewAggregatorJobIntegrationTest {
    ...
    void validEvents_shouldProduceAggregatedOutput() throws Exception {
        ...
        List<PageviewEvent> events = Arrays.asList(
                new PageviewEvent(1L, "SW19", "/home", 1700000000L),
                new PageviewEvent(2L, "SW19", "/about", 1700000010L),
                new PageviewEvent(3L, "E14", "/contact", 1700000020L)
        );

        DataStream<PageviewEvent> source = env.fromCollection(events);
    ...
    void invalidTimestamp_shouldRouteToDlqSink() throws Exception {
        ...
        List<PageviewEvent> events = Arrays.asList(
                new PageviewEvent(1L, "SW19", "/home", 1700000000L),       // valid
                new PageviewEvent(2L, "E14",  "/about", 1_500_000_000L),   // pre-2020 — DLQ
                new PageviewEvent(3L, "N1",   "/shop",  (System.currentTimeMillis() / 1000L) + 7_200L) // far future — DLQ
        );
    ...
    void nullPostcode_shouldRouteToDlqSink() throws Exception {
        ...
        /* null postcode is impossible with avro's non-nullable string type — it would fail
         * at serialization time. empty and blank strings test the same validation path. */
        List<PageviewEvent> events = Arrays.asList(
                new PageviewEvent(1L, "SW19", "/home", 1700000000L),
                new PageviewEvent(2L, "", "/about", 1700000010L),
                new PageviewEvent(3L, "  ", "/contact", 1700000020L)
        );
